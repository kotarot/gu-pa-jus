...
| `1Z000004`   | シンタックスエラーでコンパイルできない。 | 答えを直接書いているのでだめ。※意図しない検出（誤検出）の可能性もあるので後で手動で確認する。 | コンパイルは通るけど実行でセグフォる。 | N/A |
| `1Z000005_1` | ファイル名を1文字間違えている (`areb.c`)。※単なる提出者のミスの可能性があるので後で手動で確認する。 |  |  | N/A |
| `1Z000005_2` | N/A | 正しい。 | N/A | N/A |
| `1Z000006`   | 使用禁止語句 (課題ごとにカスタマイズ定義できるがここでは `system` という語句を指定しているとする) を使用している。※意図しない検出（誤検出）の可能性もあるので後で手動で確認する。 | N/A | 存在しない外部ファイルを開こうとしている。 | N/A |
