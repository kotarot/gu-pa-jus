...
- `1Z000004`: `area.c` はシンタックスエラーでコンパイルできない。`circle.c` は答えを直接書いているためだめ。※意図しない検出の可能性もあるので後で手動で確認する。
- `1Z000005_1`: (`1Z000005` の1回目の提出を想定) `area.c` のファイル名を間違えている (`areb.c`)。※単なるミスの可能性があるので後で手動で確認する。
- `1Z000005_2`: (`1Z000005` の2回目の提出を想定) `circle.c` は正しい。
